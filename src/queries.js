...
function addRole() {
  connection_1.default.query("SELECT * FROM department", function (err, res) {
    if (err) throw err;
    var departments = res.rows.map(function (department) {
      return {
...
function addEmployee() {
  connection_1.default.query("SELECT * FROM role", function (err, res) {
    if (err) throw err;
    var roles = res.rows.map(function (role) {
      return {
        name: role.title,
        value: role.id,
      };
    ...
    connection_1.default.query("SELECT * FROM employee", function (err, res) {
      if (err) throw err;
      var managers = res.rows.map(function (employee) {
        return {
...
function updateEmployeeRole() {
  connection_1.default.query("SELECT * FROM employee", function (err, res) {
    if (err) throw err;
    var employees = res.rows.map(function (employee) {
      return {
        name: "".concat(employee.first_name, " ").concat(employee.last_name),
        value: employee.id,
      };
